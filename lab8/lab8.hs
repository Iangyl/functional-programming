...
showTerm term var = (if fst term < 0 then "(" else "") ++ (if fst term == -1 then "-" else "") ++ (if fst term > 1 || fst term < -1 then show (fst term) else "") ++ var ++ (if snd term /= 1 then "^" ++ show (snd term) else "") ++ (if fst term < 0 then ")" else "")
